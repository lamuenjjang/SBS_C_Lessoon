...
#pragma region 주소연산자
	...
	// 의 시작 주소를 의미하며, 메모리 공간은 1byte
	// 의 크기로 나누어 표현합니다.
#pragma endregion

#pragma region scanf함수
	// 표준 입력 함수로, 여러종류의 데이터를 다양한
	// 서식에 맞추어 입력해주는 함수입니다.

	// int score = 0;

	// 표준 입력함수는 입력을 수행할 때까지 다음
	// 작업으로 넘어갈 수 없습니다.
	// scanf_s("%d\n", &score);
...
#pragma region 포인터
	...
	*pointer = 15.5f;
	...
	printf("pointer의 변수의 값 : %f\n", pointer);

	// 포인터 변수를 저장하기 위해 주소값을 저장할 변수를
	// 자료형과 포인터 변수의 자료형이 일치해야 합니다.

	printf("pointer 변수의 크기 : %d\n", sizeof(pointer));

	// 포인터 변수의 크기는 중앙처리 장치가 한 번에 처리할 수
